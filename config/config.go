...
func LoadConfig(path string) Iconfig {
	...
	return &config{
		...
		db: &db{
			...
			elasticHost: envMap["DB_ELASTIC_HOST"],
			elasticPort: func() int {
				p, err := strconv.Atoi(envMap["DB_ELASTIC_PORT"])
				if err != nil {
					log.Fatalf("Load Port DB Failed: %v", err)
				}
				return p
			}(),
			elasticDebug: cast.ToBool(envMap["DB_ELASTIC_DEBUG"]),
		},
		jwt: &jwt{
...
func (d *db) Url() string {
	...
	return fmt.Sprintf("sqlserver://%s:%s@%s:%d?database=%s&sslmode=%s", d.username, d.password, d.host, d.port, d.database, d.sslMode)
}
...
func (d *db) ElasticURL() string {
	return fmt.Sprintf("http://%s:%d", d.elasticHost, d.elasticPort)
}

func (d *db) ElasticDebug() bool {
	return d.elasticDebug
}
